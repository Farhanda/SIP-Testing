  ...
  return [
    {
      ...
      expectedStatus: 'Sukses — header & filter tampil', expectedResponse: 'Heading "Dashboard Overview", tombol "Export report", section "Trending topic" & "Search filters", tombol Apply/Reset tampil',
      specTitle: 'halaman dashboard menampilkan header, filter, dan tombol aksi',
      assertions: 'heading & tombol aksi visible; "Trending topic" & "Search filters" headings visible',
      source: 'Hardcoded di spec', notes: 'Bug A2: tombol "Export report" tanpa onClick (tidak melakukan apa-apa)',
    },
    {
    ...
    },
    {
      ...
      expectedStatus: 'Sukses — tema berubah', expectedResponse: 'Setelah klik toggle: wrapper [data-theme="dark"], tombol berubah "Enable light mode", warna latar wrapper berubah (--color-bg #f2f2f3 → #16181a)',
      specTitle: 'toggle dark mode harus mengubah tema visual aplikasi',
      assertions: '[data-theme="dark"] visible; button "Enable light mode" visible; computed bg wrapper berubah',
    ...
    },
    {
      ...
      expectedStatus: 'Sukses — export memicu efek', expectedResponse: 'Setelah klik Export report: terjadi download file ATAU request API baru (generate report)',
      specTitle: 'REGRESI A2: tombol Export report harus memicu aksi (download/request)',
      assertions: 'snapshot effects (download + request /api/) sebelum klik; klik → expect.poll effects.length > baseline',
      source: 'Hardcoded di spec', notes: 'Bug A2: tombol tanpa onClick sama sekali (no-op). Test FAIL sampai fitur export diimplementasikan.',
    },
    {
...
function buildKeywordCases() {
  ...
  return [
    ...
    },
    {
      ...
      expectedStatus: 'Sukses — detail & kartu analisis tampil (G2)', expectedResponse: 'Breadcrumb "Keyword monitoring"; label "Keyword on demand · Completed"; h1 keyword; status Completed; kartu dashboard render (Collection #SIP-<keyword>, Sentiment & Emotion, Content Intelligence, Top Performers); tombol Export report',
...
function buildBeDashboardCases() {
  ...
  return [
    ...
    // GAP: endpoint dipanggil FE dashboard tapi belum ada di Go service (audit FE-BE 2026-08-18).
    // Lihat tests/be/dashboard/missing-endpoints.spec.ts — saat Go sudah implement,
    // ubah ekspektasi ke 200 + struktur kontrak FE (tertera di kolom expectedResponse).
    {
      id: 'TC-BE-D89', name: 'GET /v1/dashboard/trending-topic → 404 (gap: dipanggil FE, belum ada di Go)', category: 'Gap', priority: 'High',
      ...
      precondition: `BE lokal berjalan di ${BASE_URL_BE}; FE dashboard memanggil endpoint ini`,
      expectedStatus: '404 Not Found (saat ini)', expectedResponse: 'Saat Go implement: { data: [{ id, topic, volume, delta }], meta: { period, total, generated_at } }',
      specTitle: 'GET /v1/dashboard/trending-topic → 404 (endpoint belum diimplementasikan di Go)',
      assertions: 'res.status() = 404; body plain text "404 page not found" (bukan hang/500)',
      source: 'Hardcoded di spec', notes: 'GAP: FE halaman dashboard menampilkan error "Failed to load trending topic." sampai endpoint Go tersedia',
    },
    {
      id: 'TC-BE-D90', name: 'GET /v1/dashboard/emotion-map → 404 (gap: dipanggil FE, belum ada di Go)', category: 'Gap', priority: 'High',
      method: 'GET', endpoint: `${BE_API_PREFIX}/dashboard/emotion-map?keyword=layanan-publik&period=7d`, headers: 'Accept: application/json', params: 'keyword & period', requestBody: '—',
      precondition: `BE lokal berjalan di ${BASE_URL_BE}; FE dashboard memanggil endpoint ini`,
      expectedStatus: '404 Not Found (saat ini)', expectedResponse: 'Saat Go implement: { data: { anger, neutral, fear, joy, sadness }, meta: { generated_at } }',
      specTitle: 'GET /v1/dashboard/emotion-map → 404 (endpoint belum diimplementasikan di Go)',
      assertions: 'res.status() = 404; body plain text "404 page not found" (bukan hang/500)',
      source: 'Hardcoded di spec', notes: 'GAP: kartu Emotion map di FE error state sampai endpoint Go tersedia',
    },
    {
      id: 'TC-BE-D91', name: 'GET /v1/dashboard/sentiment-map → 404 (gap: dipanggil FE, belum ada di Go)', category: 'Gap', priority: 'High',
      method: 'GET', endpoint: `${BE_API_PREFIX}/dashboard/sentiment-map?keyword=layanan-publik&period=7d`, headers: 'Accept: application/json', params: 'keyword & period', requestBody: '—',
      precondition: `BE lokal berjalan di ${BASE_URL_BE}; FE dashboard memanggil endpoint ini`,
      expectedStatus: '404 Not Found (saat ini)', expectedResponse: 'Saat Go implement: { data: { positive, neutral, negative }, meta: { generated_at } }',
      specTitle: 'GET /v1/dashboard/sentiment-map → 404 (endpoint belum diimplementasikan di Go)',
      assertions: 'res.status() = 404; body plain text "404 page not found" (bukan hang/500)',
      source: 'Hardcoded di spec', notes: 'GAP: kartu Sentiment map di FE error state sampai endpoint Go tersedia',
    },
    {
      id: 'TC-BE-D92', name: 'GET /v1/dashboard/sentiment-trend → 404 (gap: dipanggil FE, belum ada di Go)', category: 'Gap', priority: 'High',
      method: 'GET', endpoint: `${BE_API_PREFIX}/dashboard/sentiment-trend?keyword=layanan-publik&period=7d`, headers: 'Accept: application/json', params: 'keyword & period', requestBody: '—',
      precondition: `BE lokal berjalan di ${BASE_URL_BE}; FE dashboard memanggil endpoint ini`,
      expectedStatus: '404 Not Found (saat ini)', expectedResponse: 'Saat Go implement: { data: [{ date, label, positive, negative }], meta: { generated_at } }',
      specTitle: 'GET /v1/dashboard/sentiment-trend → 404 (endpoint belum diimplementasikan di Go)',
      assertions: 'res.status() = 404; body plain text "404 page not found" (bukan hang/500)',
      source: 'Hardcoded di spec', notes: 'GAP: kartu Sentiment trend di FE error state sampai endpoint Go tersedia',
    },
    {
      id: 'TC-BE-D93', name: 'GET /v1/dashboard/sentiment-trend-hourly → 404 (gap: dipanggil FE, belum ada di Go)', category: 'Gap', priority: 'Medium',
      method: 'GET', endpoint: `${BE_API_PREFIX}/dashboard/sentiment-trend-hourly?keyword=layanan-publik&period=7d&date=2026-08-18`, headers: 'Accept: application/json', params: 'keyword, period & date', requestBody: '—',
      precondition: `BE lokal berjalan di ${BASE_URL_BE}; FE dashboard memanggil endpoint ini`,
      expectedStatus: '404 Not Found (saat ini)', expectedResponse: 'Saat Go implement: { data: [{ hour, label, positive, negative }], meta: { generated_at, date } }',
      specTitle: 'GET /v1/dashboard/sentiment-trend-hourly → 404 (endpoint belum diimplementasikan di Go)',
      assertions: 'res.status() = 404; body plain text "404 page not found" (bukan hang/500)',
      source: 'Hardcoded di spec', notes: 'GAP: detail hourly Sentiment trend di FE error sampai endpoint Go tersedia',
    },
  ];
}
...
const PLATFORM_BE = {
  ...
  notes: [
    ['Sumber kebenaran', 'Struktur respons mengikuti Swagger BE & verifikasi langsung ke BE lokal. Status 2026-08-18: 10 endpoint aktif — /health/live, /health/ready, /v1/dashboard/summary, conversation-trend, conversation-trend-hourly, top-accounts, top-hashtags, top-posts, topic-intelligence, topic-intelligence-detail'],
    ...
    ['Endpoint dihapus', 'Users/Tasks/Files/Provider sudah tidak ada di BE (2026-08-14) → test & test case terkait dihapus'],
    ['Endpoint terbaru', 'top-accounts, top-hashtags, top-posts, topic-intelligence & topic-intelligence-detail ditambahkan 2026-08-18 (tests/be/dashboard/top.spec.ts & topic.spec.ts) — termasuk quirk stats scope penuh & clamp pagination'],
  ],
};
